# TorchNexus Agent 使用指南

TorchNexus Agent 用于将《火炬之光：无限》的游戏连接通过本机转发并上传记录，以便在 TorchNexus 中查看跑图、掉落与统计数据。

官网与用户中心：[https://torchnexus.cc](https://torchnexus.cc)
...
需要 Android SDK、NDK、Java 21、Rust 稳定版、`cargo-ndk 2.6.0` 与 `uniffi-bindgen 0.32.0`。设置 `ANDROID_HOME`（并确保可找到 NDK）后，在 Windows 运行：
